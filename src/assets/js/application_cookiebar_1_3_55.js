/**
 * Original: https://www.gov.br/portal_javascripts/++resource++govbr.policy/application_cookiebar_1_3_55.js?original=1
 */
var habilitar = false
var idioma = ''
var mensagens = ''
...
function urlConfiguracao() {
    ...
    if (url_array[1] == 'pt-br' || url_array[1] == 'en' || url_array[1] == 'es') {
        baseUrl = baseUrl + '/' + url_array[1];
    }
    if (url_array[2] == 'pt-br' || url_array[2] == 'en' || url_array[2] == 'es') {
        if (portal_url) {
            baseUrl = baseUrl + '/' + url_array[2];
        } else {
            baseUrl = baseUrl + '/' + url_array[1] + '/' + url_array[2];
        }
    }
    baseUrl = baseUrl + '/js/configuracoes_cookiebar_govbr.json';
    return baseUrl
}
...
function _redefinirCookies() {
    fetch(config_url)
        ...
        .then((out) => {
            ...
            if (habilitar) {
                const defaultInfoList = mensagens
                ...
                defaultInfoList.forEach(defaultInfo => {
                    ...
                    defaultInfo.cookieGroups.forEach((group, groupIndex) => {
                        try {
                            group.groupSelected = outputInfo.g[groupIndex].on
                        } catch (error) {}
                        group.cookieList.forEach((cookie, cookieIndex) => {
                            try {
                                cookie.cookieSelected = outputInfo.g[groupIndex].on
                            } catch (error) {}
                        })
                    })
                })
