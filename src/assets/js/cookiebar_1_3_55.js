/**
 * A classe BRCookiebar é responsável por gerenciar a barra de cookies do site.
 * Ela tem funções que permitem configurar a barra de cookies, como definir o comportamento,
 * mostrar ou ocultar a barra, definir botões de rejeição, aceitação e fechamento, e criar informações
 * de cookies para exibição. Além disso, a classe também lida com a interação do usuário com a barra de
 * cookies, como selecionar ou desmarcar todos os cookies, e ajustar a barra de cookies com base no tamanho da janela.
 *
 * Original: https://www.gov.br/portal_javascripts/++resource++govbr.policy/cookiebar_1_3_55.js?original=1
 */
...
class BRCookiebar {
    ...
    _showCookiebar() {
        ...
        if (this.mode === 'open') {
            ...
            this.component
                .querySelectorAll('.actions .br-button.btn-manage')
                .forEach((button) => {
    ...
    _hideCookiebar() {
        ...
        // govbr
        // reset-cookies
        document.querySelectorAll('.reset-cookies').forEach((reset_cookies) => {
            reset_cookies.classList.remove('d-none')
        })
    }

    _setRejectButton() {
        this.component
            .querySelectorAll('.actions .br-button.reject-all')
            .forEach((button) => {
                button.innerHTML = this.data.rejectButton || 'Rejeitar todos';
            });

        // const html = `<button class="br-button secondary small reject-all" type="button" aria-label="${this.data.rejectButton}">${this.data.rejectButton}</button>`

        // const rejectButton = new DOMParser().parseFromString(html, 'text/html')
        //     .body.firstElementChild

        // if (this.mode === 'open') {
        //     // rejectButton.classList.remove('secondary')
        //     this.component
        //         .querySelector('.br-modal-footer.actions')
        //         .prepend(rejectButton)
        // } else {
        //     this.component.querySelector('.br-modal-body').prepend(rejectButton)
        // }
    }

    // _moveRejectButton(from, to) {
    //     const rejectButton = document.querySelector(
    //         from + ' ' + '.br-button.reject-all'
    //     )
    //     const destiny = document.querySelector(to)
    //     destiny.prepend(rejectButton)
    // }

    // Seta o label do botÃ£o primÃ¡rio
    // Adiciona/remove classe block de acordo com o viewport
    // Foca o botÃ£o no tab quando no modo default
    _setPrimaryButton() {
        this.component
            .querySelectorAll('.actions .br-button.btn-accept')
            .forEach((button) => {
                this.data.acceptLabel ?
                    (button.innerHTML = this.data.acceptLabel) :
            ...
            .forEach((button) => {
                ...
                this.data.allOptOut ?
                    ...
                    this.data.optInButton ?
                        ...
                        button.setAttribute('aria-label', 'Ver PolÃ­tica de Cookies')
                ...
                button.addEventListener('click', () => {
                    this.component.classList.remove('default')
                    button.classList.add('d-none')
                    // this._moveRejectButton(
                    //     '.br-modal-body',
                    //     '.br-modal-footer.actions'
                    // )
                    this.component.focus()
                    document.body.style.overflowY = 'hidden'
                    this._setOpenView()
                    ...
                    // govbr
                    // altera botao para salvar
                    document.querySelectorAll('.br-button.btn-accept')
                        .forEach((button) => {
            ...
            .forEach((closeButton) => {
                ...
                closeButton.addEventListener('click', () => {
                    ...
                    if (this.mode === 'open') {
                        ...
                        //     )
                        // govbr
                        // mantem o valor inicial groupSelected
                        // pois ainda nao existe o cookie lgpd-cookie-v2 gravado
                        fetch(config_url)
                            .then(res => res.json())
                            .then((out) => {
                                var msgs = out['mensagens']
                                var initial_config = msgs[0]
                                var groups = initial_config.cookieGroups
                                for (var i = 0; i < groups.length; i++) {
                                    var group_selected = groups[i].groupSelected
                                    ...
                                    cookie_list.forEach(cookie => {
                                        cookie.cookieSelected = group_selected
                                    })
                                    var checkbox_id = `check-group-${i}`
                                    var checkbox = document.getElementById(checkbox_id)
                                    if (checkbox && group_selected) {
                                        ...
                                        checkbox.setAttribute('aria-label', this.data.unselectAllGroupLabel ? this.data.unselectAllGroupLabel : 'Desselecionar toda classe')
                                    } else if (checkbox && !group_selected) {
                    ...
                    this.component
                        .querySelector('.actions .br-button.btn-manage')
                        .classList.remove('d-none')
                    document.body.style.overflowY = 'auto'
                    this._setCloseView()

                    // govbr
                    // altera botao para aceitar
                    document.querySelectorAll('.br-button.btn-accept')
                        .forEach((button) => {
                            button.innerHTML = this.data.acceptLabel
                            button.setAttribute('aria-label', this.data.acceptLabel)
    ...
    _setOpenView() {
        ...
        const padding = window
            ...
            .match(/\d+/)
        const height = `${window.innerHeight - padding * 2 - modalFooter.offsetHeight
            }px`
    ...
    _setInfoText() {
        this.component
            ...
            .forEach((infoText) => {
                infoText.innerHTML = this.data.infoText.replace("url_politica_privacidade", url_politica_privacidade)
            })
    }
        ...
        this.component
            ...
            .forEach((mainTitle) => {
                const title = `<div class="row">
                        ...
                        <button class="br-button close circle small float-right" type="button" data-dismiss="br-modal" aria-label="${this.data.closeLabel
    ...
    _setListHeader() {
        this.component
            ...
            .forEach((mainContent) => {
                const listHeader = `<div class="header d-none">
                              <div class="row justify-content-between flex-fill">
                                <div class="col-12 col-sm align-self-center mb-2">
                                  <div class="title">${this.data.cookieGroupsLabel
                        ...
                        ? `<span class="feedback warning" role="alert">${this.data.allAlertMessage}</span>`
                        : ''
                    }
                                </div>
                              </div>
    ...
    _setEntryText() {
        this.component
            ...
            .forEach((entryText) => {
                entryText.innerHTML = this.data.entryText.replace("url_politica_privacidade", url_politica_privacidade)
            })
    }
                ...
                this.data.cookieGroups.forEach((item, index) => {
                    const group = `
                      <div class="br-item group-info">
                        <div class="row mb-1">
                          <div>
                            <span class="group-name" title="Expandir">${item.groupName
                        }</span>
                          </div>
                          <div>
                            ${item.groupOptOut
                        ...
                        <div class="row">
                          <div class="col-12 message mt-1 mb-1">
                            ${(!item.groupSelected ||
                            item.groupIndeterminate) &&
                            item.groupAlertMessage
                            ? `<span class="feedback warning" role="alert">${item.groupAlertMessage}<a href="#" class="feedback-warning-close" onclick="this.parentNode.remove()return false;">x</a></span>`
                            : ''
                        }
                          </div>
                        </div>
    ...
    _setNotification() {
        if (this.data.noteList && this.data.noteList.length > 0) {
            this.component
                ...
                .forEach((mainContent) => {
                    const note = `<hr>
                      <div class="br-item open">
                      <div class="row">
                        <div>
                          <span class="group-name" title="Expandir">${this.data.noteTitle
                        }</span>
                        </div>
                        <div class="col-auto">
    ...
    _setLinks() {
        if (this.data.links && this.data.links.length > 0) {
            this.component.querySelectorAll('.br-modal-body').forEach((modalHeader) => {
    ...
    _buildCookieInfo(groupIndex, cookieList) {
        ...
        cookieList.forEach((cookie, index) => {
            cookieInfo += `<div class="br-card">
                    ...
                    ? `<span class="feedback warning" role="alert">${this.data.cookieGroups[groupIndex].cookieList[index].alertMessage}<a href="#" class="feedback-warning-close" onclick="this.parentNode.remove()return false;">x</a></span>`
                    : ''
                }
                          </div>
                        </div>
                        <div class="row">
                          <div class="fixed-width cookie-term"><span>${this.data.cookieNameLabel
                    ? this.data.cookieNameLabel
                    : 'Cookies'
                }</span></div>
                          <div class="col-12 col-sm mb-1 cookie-data"><span>${cookie.cookieName
                }</span></div>
                        </div>
                        <div class="row">
                          <div class="fixed-width cookie-term"><span>${this.data.expiresLabel
                    ? this.data.expiresLabel
                    : 'Vencimento'
                }</span></div>
                          <div class="col-12 col-sm mb-1 cookie-data"><span>${cookie.expires
                }</span></div>
                        </div>
                        <div class="row">
                          <div class="fixed-width cookie-term"><span>${this.data.domainLabel
                    ? this.data.domainLabel
                    : 'DomÃ­nio'
                }</span></div>
                          <div class="col-12 col-sm mb-1 cookie-data"><span>${cookie.domain
                }</span></div>
                        </div>
                        <div class="row">
                          <div class="fixed-width cookie-term"><span>${this.data.enterpriseLabel
                    ? this.data.enterpriseLabel
                    : 'Empresa'
                }</span></div>
                          <div class="col-12 col-sm mb-1 cookie-data"><span>${cookie.entreprise
                }</span></div>
                        </div>
                        <div class="row">
                          <div class="fixed-width cookie-term"><span>${this.data.purposeLabel
                    ? this.data.purposeLabel
                    : 'Finalidade'
                }</span></div>
                          <div class="col-12 col-sm mb-1 cookie-data"><span>${cookie.purpose
                }</span></div>
                        </div>
                        <div class="row">
                          <div class="fixed-width cookie-term"><span>${this.data.descriptionLabel
                    ? this.data.descriptionLabel
                    : 'DescriÃ§Ã£o'
                }</span></div>
                          <div class="col-12 col-sm mb-1 cookie-data"><span>${cookie.description
                }</span></div>
                        </div>
                      </div>
                    </div>`
    ...
    _createCheckAll(checked, indeterminate) {
        const checkAll = `<div class="br-checkbox-govbr">
    ...
    _createCheckGroup(index, piece, checked, indeterminate, disabled) {
        const checkGroup = `<div class="br-checkbox-govbr">
                          <input id="check-${piece + index}" name="check-${piece + index
            }" type="checkbox" aria-label="${this._setCheckGroupLabel(
                checked,
                indeterminate
            )}" ${checked ? 'checked' : ''} ${indeterminate ? 'indeterminate' : ''
            } ${disabled ? 'disabled' : ''} tabindex="0"/>
                          <label for="check-${piece + index
    ...
    _createSwitchButton(index, piece, selected) {
        const switchButton = `<div class="br-switch small icon">
                            <input id="switch-${piece + index
            }" type="checkbox" ${selected ? 'checked' : ''
            } tabindex="-1">
                            <label for="switch-${piece + index}" aria-label="${selected
                ? this.data.onLabel
                    ? this.data.onLabel
                    : 'Ligado'
                : this.data.offLabel
                    ...
                    : 'Desligado'
                            ...
                            <div class="switch-data" data-enabled="${this.data.onLabel ? this.data.onLabel : 'Ligado'
            }" data-disabled="${this.data.offLabel ? this.data.offLabel : 'Desligado'
            }"></div>
                          </div>`
    ...
    _insertAllSelectedAlertMessage() {
        ...
        if (this.data.allAlertMessage && this.data.allAlertMessage !== '') {
            const messageTemplate = `<span class="feedback warning" role="alert">${this.data.allAlertMessage}<a href="#" class="feedback-warning-close" onclick="this.parentNode.remove()return false;">x</a></span>`
    ...
    _insertGroupAlertMessage(indexGroup) {
        ...
        if (
            ...
            const messageTemplate = `<span class="feedback warning" role="alert">${this.data.cookieGroups[indexGroup].groupAlertMessage}<a href="#" class="feedback-warning-close" onclick="this.parentNode.remove()return false;">x</a></span>`
    ...
    _insertAlertMessage(indexGroup, indexCookie) {
        if (
            ...
            const messageTemplate = `<span class="feedback warning" role="alert">${this.data.cookieGroups[indexGroup].cookieList[indexCookie].alertMessage}<a href="#" class="feedback-warning-close" onclick="this.parentNode.remove();return false;">x</a></span>`
        ...
        for (const [indexGr, group] of this.component
            ...
            if (indexGr === indexGroup) {
                const feedback = group.querySelector(
                    '.row:nth-child(2) div:nth-child(1) .feedback'
                )
                if (feedback) {
                    feedback.remove()
                }
    ...
    _setCheckAll() {
        this.component
            .querySelectorAll(
                '.main-content .header .br-checkbox-govbr input[type="checkbox"]'
            )
            .forEach((checkAll) => {
                checkAll.addEventListener('click', () => {
                    ...
                    this.component
                        .querySelectorAll(
                            '.main-content .group-info .br-checkbox-govbr input[type="checkbox"]'
                        )
                        .forEach((checkGroup) => {
    ...
    _setCheckGroup() {
        this.component
            ...
            .forEach((group, groupIndex) => {
                group
                    .querySelectorAll('.br-checkbox-govbr input[type="checkbox"]')
                    .forEach((checkGroup) => {
    ...
    _setAcceptBehavior() {
        const acceptButton = this.component.querySelector(
            '.actions .br-button.btn-accept'
        )
        acceptButton.addEventListener('click', () => {
            // Habilita todos os checkboxes quando fechado
            if (this.component.classList.contains('default')) {
                this.component
                    .querySelectorAll(
                        '.group-info input[type="checkbox"]:not([disabled])'
                    )
                    .forEach((checkbox) => {
    ...
    _setRejectBehavior() {
        const rejectButton = this.component.querySelector(
            '.br-button.reject-all'
        )
    ...
    _setOutputJSON() {
        this.output = {}
        // 'v' indica versÃ£o do cookie banner. Usada como condiÃ§Ã£o para atualizar ou nÃ£o o cookie no browser.
        // Sempre que tiver alteraÃ§Ã£o no cookie, incrementar a versÃ£ para o browser atualizar o cookie automaticamente.
        // VersÃ£o inicial igual a 1
        this.output.v = version
        this.output.g = []
        this.data.cookieGroups.forEach((group) => {
            const cookies = []
            group.cookieList.forEach((cookie) => {
                cookies.push(cookie.cookieId)
            })
            localStorage.setItem(group.groupId, JSON.stringify(cookies));
            this.output.g.push({
                id: group.groupId,
                on: group.groupIndeterminate
                    ? 'indeterminate'
                    : group.groupSelected
                        ? true
                        : false,
    ...
    _resetCookiebar() {
        ...
        const modalFooter = `<div class="br-modal-footer actions">
        ...
        document
            .querySelectorAll('.br-cookiebar .br-modal .br-card > div:first-child .br-modal-footer')
            .forEach((item) => {
                item.parentElement.insertAdjacentHTML('beforeend', modalFooter)
                item.remove()
            })
...
// FunÃ§Ã£o criada para ser chamada sempre que a versÃ£o do cookie for alterada.
// Incrementar a versÃ£o "v" no cookie para o browser atualizar automaticamente
function _updateOutputJSON(json) {
    var cookie_v2 = ('; ' + document.cookie).split(`lgpd-cookie-v2=`).pop().split(';')[0];
    if (cookie_v2) {
        var json_cookie_v2 = JSON.parse(cookie_v2);
        if (json_cookie_v2) {
            var groups_cookie = json_cookie_v2['g'];
            var groups_default = json['cookieGroups'];
            // Quando atualizar o cookie, as opÃ§Ãµes jÃ¡ selecionadas devem permanecer
            // Loop nos grupos e nos cookies de cada lista setando o valor jÃ¡ selecionado no browser
            for (var i = 0, li = groups_cookie.length; i < li; i++) {
                for (var j = 0, lj = groups_default.length; j < lj; j++) {
                    if (groups_cookie[i]['id'] == groups_default[j]['groupId']) {
                        var group_selected = false
                        if (groups_cookie[i]['on'] === 1 || groups_cookie[i]['on'] === 0) {
                            group_selected = groups_cookie[i]['on'] == 1 ? true : false
                        }
                        if (groups_cookie[i]['on'] === true || groups_cookie[i]['on'] === false) {
                            group_selected = groups_cookie[i]['on'] == true ? true : false
                        }
                        json['cookieGroups'][j]['groupSelected'] = group_selected
                        var cookies = groups_default[i]['cookieList']
                        for (var k = 0, lk = cookies.length; k < lk; k++) {
                            json['cookieGroups'][i]['cookieList'][k]['cookieSelected'] = group_selected
                        }
                    }
                }
            }
    ...
    output.v = version
    output.g = []
    json.cookieGroups.forEach((group) => {
        const cookies = []
        group.cookieList.forEach((cookie) => {
            cookies.push(cookie.cookieId)
        })
...
document.addEventListener('DOMContentLoaded', function () {
    // Desabilita o cookie antigo "lgpd_cookie_status"
    document.cookie = 'lgpd_cookie_status= ; expires=Thu, 01 Jan 2000 00:00:01 GMT; path=/';
    // BRCookiebar
    var habilitar = false;
    var idioma = '';
    var mensagens = '';
    ...
    fetch(config_url)
        ...
        .then((out) => {
            ...
            cookiename = 'lgpd-cookie-v2';
            if (habilitar) {
                var cookieinformation = getCookie(cookiename);
                if (typeof cookieinformation !== 'undefined') {
                    // Verifica a versÃ£o do cookie para saber se precisa atualizar
                    // VersÃ£o inicial igual a 1
                    var versao = JSON.parse(cookieinformation)['v'];
                    if (versao != version) {
                        var json = {}
                        var json_mensagens = mensagens
                        if (json_mensagens) {
                            var mensagens_idioma = json_mensagens.filter((obj) => {
                                return obj.lang === idioma
                            })
                            if (mensagens_idioma.length > 0) {
                                json = mensagens_idioma[0]
                            } else {
                                json = json_mensagens[0]
                            }
                        }
                        if (json != {} && json != 'undefined') {
                            // Grava o cookie respeitando as opÃ§Ãµes jÃ¡ selecionadas
                            gravarCookie(_updateOutputJSON(json));
                            window.location = window.location.href;
                        }
                    }
                    _createEventIcone();
                    ...
                    novaCookieBar(idioma, JSON.stringify(mensagens), 'default');
                }
            }
        }).catch(err => console.error(err));

    if (!getCookie('lgpd-cookie-v2')) {
        var resetCookiesElement = document.querySelector('.reset-cookies');
        if (resetCookiesElement) {
            resetCookiesElement.classList.add('d-none');
        }
    }
